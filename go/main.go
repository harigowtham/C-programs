...
func calroomrange(max int32, min int32, array []int32) {
	if(max == 0) {
		max = int32(float32(min)*.75) + min
		min = int32(float32(min)*.75)
	} else if (min == 0) {
		min = int32(float32(max)*.75) + max
...
func calbudgetrange(max int32, min int32, array []int32) {
	...
	array[0] = int32(float32(min)*0.3)
	array[1] = int32(float32(min)*0.6)
	array[2] = min
	array[3] = max
...
func (s *Search) callroutine(i,j,k,arrays,restcontri, p *Plist) {
	//create a temp list
	...
	for i := 2; i >= 0; i-- {
		for j := 1; j>= 0; j-- {
			for k := 1; k>= 0; k-- {
				...
				fmt.Printf("%+v\n", *currentNode)
				currentNode.Percentage = contri
				for currentNode.next != nil {
					currentNode = currentNode.next
					fmt.Printf("%+v\n", *currentNode)
					currentNode.Percentage = contri+1
					PriorityQueue[i] = &currentNode
					//heap.Push(PriorityQueue, currentNode)
				}*/

				//s.callroutine(i,j,k, arrays, contri, p)
			}
		}
	}
}
